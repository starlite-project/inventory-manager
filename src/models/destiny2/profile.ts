...
import { Base, type IconURL } from '../base';
import { type RawUserInfoCard, UserInfoCard } from '../user/info';
...
export class DestinyProfileUserInfoCard
	extends Base<RawDestinyProfileUserInfoCard>
	implements IconURL
{
	public iconURL(): URL {
...
export class DestinyLinkedProfilesResponse extends Base<RawDestinyLinkedProfilesResponse> {
	public get profiles(): DestinyProfileUserInfoCard[] {
		return this._data.profiles.map(
			(profile): DestinyProfileUserInfoCard =>
				new DestinyProfileUserInfoCard(profile)
		);
	}
